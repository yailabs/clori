...
| Application serving | Local protocol v17 and YVEX OpenAI Compatibility Profile v2 share one persistent host, engine manager, session registry, request routing, and telemetry authority; bounded session fork composes immutable prefix sharing with independent semantic state. Protocol v17 carries separate engine-kind/execution-strategy facts, typed first/last image conditions, identity-bearing media trajectory, canvas, duration and seed selection, plus explicit/adaptive completion-envelope ownership without changing text routing. |
