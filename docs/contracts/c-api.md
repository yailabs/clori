...
an older peer, so every non-v14 frame refuses during the handshake;
...
Version 14 removes the mixed public generation-mode axis. Engine records now
identify text versus media independently from target-only versus speculative
text execution. DSpark remains a DeepSeek implementation of the semantic
speculative strategy below the server boundary. The incompatible engine and
event layouts advance to schemas v2 and v4; v13 frames and legacy record
schemas fail closed rather than being reinterpreted.
