...
Canonical source: `$HOME/lab/models/hf/deepseek/DeepSeek-V4-Flash`; v0.1.0
target: `deepseek4-v4-flash` on DGX Spark CUDA. Snapshot `60d8d70770c6776ff598c94bb586a859a38244f1`
binds 46 shards and 69,187 records.

## Current Boundary

YVEX admits the selected complete GGUF, materializes its attention bindings, and
executes complete attention through the common CPU/CUDA runtime. Sessions own
exact persistent state for all 43 layers, with atomic append/read and
clear/reuse. Tokenizer-backed prefill, transformer/MoE execution, text
generation, and a DeepSeek generation command remain unsupported.
...
./yvex graph attention state exercise \
  ...
  --operation-scope core --tokens 2 --probe canonical --scope full --output json
```

The state exercise commits real publications across all 43 layers, proves
causal read-after-write, and proves clear/reuse. It is not prompt prefill or
model decode.
