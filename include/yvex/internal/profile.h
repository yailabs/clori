...
typedef enum {
    ...
    YVEX_RUNTIME_PROFILE_GENERATED_TOKENS, YVEX_RUNTIME_PROFILE_COUNTER_COUNT
                                           ...
                                           char output[YVEX_SHA256_HEX_BYTES])
{
    ...
    if (!record || !output ||
        !yvex_sha256_update_text(&hash, "yvex.runtime.profile.v1") ||
                                        ...
                                        yvex_error *err)
{
    ...
    record->schema_version = YVEX_RUNTIME_PROFILE_SCHEMA_V1;
    record->mode = mode;
    record->scope = scope;
...
{
    if (!record || record->schema_version != YVEX_RUNTIME_PROFILE_SCHEMA_V1 ||
...
static inline const char *runtime_profile_counter_name(yvex_runtime_profile_counter counter)
{
    static const char *const names[] = {"host_payload_reads", "mapped_bytes_touched",
        ...
        "workspace_resets", "prompt_tokens", "reused_tokens", "new_prefill_tokens", "generated_tokens"};
