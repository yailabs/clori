...
{
    double sum = 0.0, correction = 0.0, verified = 0.0, verified_correction = 0.0;
    ...
    double *deviations, unsigned long long left, unsigned long long right)
{
    ...
    tokens[left] = tokens[right]; logits[left] = logits[right];
    probabilities[left] = probabilities[right]; deviations[left] = deviations[right];
    tokens[right] = token; logits[right] = logit;
    probabilities[right] = probability; deviations[right] = deviation;
}
    ...
    unsigned long long padded, unsigned int mode)
{
    ...
    for (index = count + threadIdx.x; index < padded; index += blockDim.x) {
        tokens[index] = ~0u; logits[index] = 0.0f;
        probabilities[index] = -1.0; deviations[index] = 1.0 / 0.0;
    }
    __syncthreads();
    ...
    double *deviations, unsigned long long count)
{
    ...
    for (unsigned long long read = 0ull; read < count; ++read) {
        ...
        if (write != read) {
            tokens[write] = tokens[read]; logits[write] = logits[read];
    ...
    unsigned int *selection, int *status)
{
    ...
    if (!threadIdx.x) {
        ...
        for (unsigned long long index = 0ull; index < vocabulary_size; ++index) {
            ...
            if (!isfinite(values[index]) || !isfinite(scaled)) {
                atomicCAS(status, 0, 1); break;
            }
            tokens[index] = (unsigned int)index; logits[index] = values[index];
            deviations[index] = 0.0; probabilities[index] = scaled;
            if (scaled > maximum_scaled) maximum_scaled = scaled;
            if (values[index] > maximum_logit) maximum_logit = values[index];
        }
        for (unsigned long long index = 0ull; !*status && index < vocabulary_size; ++index)
            probabilities[index] = exp(probabilities[index] - maximum_scaled);
        count = vocabulary_size;
        if (!*status && !sampling_normalize_device(
                            probabilities, count,
                            &statistics[YVEX_CUDA_SAMPLING_NORMALIZATION_ERROR]))
            atomicCAS(status, 0, 1);
        if (!*status) count = sampling_compact_positive(
        ...
        if (!*status && min_p > 0.0) {
            ...
            for (unsigned long long read = 0ull; read < count; ++read) {
                if (probabilities[read] <
                    statistics[YVEX_CUDA_SAMPLING_MIN_P_THRESHOLD]) continue;
                if (write != read) {
                    tokens[write] = tokens[read]; logits[write] = logits[read];
                    probabilities[write] = probabilities[read];
                    deviations[write] = deviations[read];
            ...
            count = write;
            if (!count || !sampling_normalize_device(
                              probabilities, count,
                              &statistics[YVEX_CUDA_SAMPLING_NORMALIZATION_ERROR]))
                atomicCAS(status, 0, 1);
        }
    ...
    if (typical_p < 1.0) {
        if (!threadIdx.x) {
            ...
            for (unsigned long long index = 0ull; index < count; ++index)
                sampling_sum_add(&entropy, &correction,
            ...
            for (unsigned long long index = 0ull; !*status && index < count; ++index)
                deviations[index] = fabs(-log(probabilities[index]) - entropy);
        }
        __syncthreads();
        if (*status) return;
        sampling_sort_device(tokens, logits, probabilities, deviations,
    ...
    __syncthreads();
    if (*status || !count) return;
    if (!threadIdx.x) counts[YVEX_CUDA_SAMPLING_TYPICAL_COUNT] = count;
    if (top_p < 1.0)
        sampling_sort_device(tokens, logits, probabilities, deviations,
                             count, padded_size, 0u);
    if (!threadIdx.x) {
    ...
    if (!threadIdx.x) {
        ...
        if (!isfinite(probabilities[selected]) || probabilities[selected] <= 0.0) {
            atomicCAS(status, 0, 1); return;
        }
        ...
        statistics[YVEX_CUDA_SAMPLING_SELECTED_PROBABILITY] = probabilities[selected];
    }
}
