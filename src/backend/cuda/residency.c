...
int yvex_cuda_resident_map_supported(const yvex_backend *backend)
{
    ...
    return state && !state->context_borrowed && backend->device_info.unified_addressing &&
           state->can_map_host &&
           (backend->pageable_uses_host_page_tables || state->host_register_readonly) &&
           state->driver.cuMemHostRegister_v2 && state->driver.cuMemHostGetDevicePointer_v2 &&
                                    ...
                                    yvex_error *err)
{
    ...
    if (rc == YVEX_OK && getenv("YVEX_TEST_CUDA_HOST_REGISTER_FAILURE")) {
        ...
        rc = yvex_cuda_status(
            ...
            state->driver.cuMemHostRegister_v2(
                (void *)(uintptr_t)host, (size_t)desc->bytes,
                YVEX_CUDA_MEMHOSTREGISTER_DEVICEMAP |
                    (state->host_register_readonly
                         ? YVEX_CUDA_MEMHOSTREGISTER_READ_ONLY : 0u)),
            "cuda.residency.register", err);
    }
    if (rc == YVEX_OK) {
        state->registered_host = (void *)(uintptr_t)host;
        state->registered_bytes = desc->bytes;
        rc = yvex_cuda_status(
            &state->driver,
            state->driver.cuMemHostGetDevicePointer_v2(
                &pointer, (void *)(uintptr_t)host, 0u),
    ...
    tensor->host_data = (unsigned char *)(uintptr_t)host;
