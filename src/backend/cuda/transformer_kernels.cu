...
{
    extern __shared__ float scratch[];
    yvex_gqa_f32_body<4u>(query, key, value, output, tokens, query_heads,
                          kv_heads, head_dim, scale, causal, scratch);
}
...
{
    extern __shared__ float scratch[];
    yvex_gqa_f32_body<8u>(query, key, value, output, tokens, query_heads,
                          kv_heads, head_dim, scale, causal, scratch);
}
