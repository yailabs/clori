        ...
        "yvex fullmodel report --model TARGET","yvex fullmodel report --model TARGET",
            "selector, path, behavior, diagnostic, transitional-layout",
                "report/materialization planning only unless subcommand proves more",yvex_fullmodel_command,
                    yvex_fullmodel_help),
    OWNED_CMD("graph","graph","mixed-transitional","Production attention and MoE graph execution.",
        "yvex graph attention ACTION|moe execute [options]",
            "yvex graph moe execute --target deepseek4-v4-flash --backend cuda --input tensor-file",
