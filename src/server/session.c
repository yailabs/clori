...
/* Purpose: admit token-by-token provider publication only when final parsing cannot
 * suppress, validate, or reinterpret generated bytes.
 * Inputs: optional sealed provider request. Effects: none.
 * Failure: returns false for null or buffered semantic modes.
 * Boundary: this selects publication timing and does not alter generated bytes. */
...
/* Purpose: project one turn failure into the stable provider-protocol error vocabulary.
 * Inputs: authoritative turn status. Effects: none.
 * Failure: unexpected statuses remain internal failures.
 * Boundary: application adapters consume the class without parsing diagnostic prose. */
...
/* Purpose: copy exact provider usage counters into one terminal protocol message.
 * Inputs: generation result, terminal message output, and error output.
 * Effects: publishes prompt/completion counts and their checked sum.
 * Failure: reports arithmetic overflow without publishing a wrapped total.
 * Boundary: prefix reuse remains telemetry and never changes standard usage. */
 ...
 * Inputs: native/provider request, vocabulary size, plan output, and error output.
 * Effects: validates and seals one sampling policy for the generation context.
 * Failure: publishes no usable plan when request options are invalid.
 * Boundary: maps intent and does not execute sampling or advance RNG. */
...
/* Purpose: select the earliest complete provider stop-string boundary across the final byte stream. */
...
/* Purpose: project one sealed generation profile into the canonical runtime event sequence.
 * Inputs: authoritative turn result and exact request correlation.
 * Effects: emits bounded phase, movement, launch, and synchronization facts without content.
 * Failure: telemetry refusal is returned to the turn publisher.
 * Boundary: events expose runtime-owned measurements and neither time nor synchronize execution. */
