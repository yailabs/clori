    ...
    *count = 0u;
    ...
    if (get_u16(header + 4u) != YVEX_LOCAL_PROTOCOL_VERSION)
        return transport_refuse(
            err, YVEX_ERR_FORMAT,
            "local protocol version is incompatible; version 7 is required");
    if (length) {
        bytes = malloc(length);
        if (!bytes)
                        ...
                        yvex_error *err)
{
    ...
    if (yvex_client_send(client, &handshake, err) != YVEX_OK ||
        ...
        response.status != YVEX_OK || strcmp(response.reason, "protocol-v7") != 0) {
        (void)close(client->fd);
        memset(client, 0, sizeof(*client));
        free(client);
        if (yvex_error_code(err) == YVEX_OK)
            yvex_error_set(err, YVEX_ERR_FORMAT, "server.protocol.handshake",
                           "daemon did not admit local protocol version 7");
        return yvex_error_code(err);
    }
