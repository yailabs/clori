...
"$YVEX_BIN" system accounts --help > "$ROOT/help.out"
grep 'usage: yvex system accounts' "$ROOT/help.out"
grep 'operation: system.accounts' "$ROOT/help.out"

YVEX_CONFIG_DIR="$ROOT/missing-config" \
YVEX_HF_CLI=/missing/hf \
YVEX_GH_CLI=/missing/gh \
  "$YVEX_BIN" system accounts status --audit > "$ROOT/status-missing.out"
...
YVEX_HF_CLI="$FAKE_HF" \
YVEX_GH_CLI="$FAKE_GH" \
  "$YVEX_BIN" system accounts providers --output table > "$ROOT/providers-table.out"
...
YVEX_HF_CLI="$FAKE_HF" \
YVEX_FAKE_HF_AUTH=1 \
  "$YVEX_BIN" system accounts whoami huggingface --audit > "$ROOT/hf-whoami.out"
...
YVEX_FAKE_HF_LOGIN_OK=1 \
  "$YVEX_BIN" system accounts login huggingface --audit > "$ROOT/hf-login.out"
...
YVEX_HF_CLI="$FAKE_HF" \
  "$YVEX_BIN" system accounts ensure huggingface --interactive never --audit > "$ROOT/hf-ensure.out" 2> "$ROOT/hf-ensure.err" && exit 1 || true
...
YVEX_FAKE_GH_LOGIN_OK=1 \
  "$YVEX_BIN" system accounts login github --audit > "$ROOT/gh-login.out"
...
YVEX_HF_CLI="$FAKE_HF" \
YVEX_GH_CLI="$FAKE_GH" \
  "$YVEX_BIN" system accounts status --audit > "$ROOT/token-status.out"
...
"$YVEX_BIN" system accounts status --output nope > "$ROOT/bad-output.out" 2> "$ROOT/bad-output.err" && exit 1 || true
