...
"$YVEX_BIN" artifact verify check --model "$MODEL" --require-token-embedding --partial-token 0 \
...
contains "$OUT_DIR/help.out" "usage: yvex artifact verify check --model FILE_OR_ALIAS"

"$YVEX_BIN" artifact verify check --model tests/fixtures/gguf/bad-magic.gguf \
...
"$YVEX_BIN" artifact verify check --model tests/fixtures/gguf/short-header.gguf \
...
"$YVEX_BIN" artifact verify check --model "$RANGE_SHORT" \
...
"$YVEX_BIN" artifact verify check --model tests/fixtures/gguf/tensor-dim-zero.gguf \
...
"$YVEX_BIN" artifact verify check --model "$MODEL_F32" --require-token-embedding --partial-token 0 \
...
"$YVEX_BIN" artifact verify check --model tests/fixtures/gguf/valid-minimal.gguf --require-token-embedding \
...
"$YVEX_BIN" artifact verify check --model "$MODEL" --partial-token 99 \
