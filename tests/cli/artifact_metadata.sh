...
"$YVEX_BIN" model registry verify "$ALIAS" --registry "$DTYPE_REG" --audit \
...
"$YVEX_BIN" model registry verify "$ALIAS" --registry "$DIMS_REG" --audit \
...
"$YVEX_BIN" model registry verify "$ALIAS" --registry "$ARCH_REG" --audit \
...
"$YVEX_BIN" model registry verify "$ALIAS" --registry "$READY_REG" --audit \
...
echo "cli inspect artifact metadata: ok"
