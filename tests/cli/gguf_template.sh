...
"$YVEX_BIN" artifact template inspect --template "$FIX" > "$OUT_DIR/inspect.out" 2> "$OUT_DIR/inspect.err" || fail "inspect failed"
...
"$YVEX_BIN" artifact template validate --template "$FIX" > "$OUT_DIR/validate.out" 2> "$OUT_DIR/validate.err" || fail "validate failed"
...
"$YVEX_BIN" artifact template --help > "$OUT_DIR/help.out" 2> "$OUT_DIR/help.err" || fail "help failed"
grep 'usage: yvex artifact template' "$OUT_DIR/help.out" >/dev/null || fail "missing help usage"
