...
"$YVEX_BIN" quant imatrix inspect --manifest "$OUT_DIR/imatrix.json" > "$OUT_DIR/inspect.out" 2> "$OUT_DIR/inspect.err" || fail "inspect failed"
...
"$YVEX_BIN" quant imatrix validate --manifest "$OUT_DIR/imatrix.json" > "$OUT_DIR/validate.out" 2> "$OUT_DIR/validate.err" || fail "validate failed"
...
"$YVEX_BIN" quant imatrix --help > "$OUT_DIR/help.out" 2> "$OUT_DIR/help.err" || fail "help failed"
grep 'usage: yvex quant imatrix' "$OUT_DIR/help.out" >/dev/null || fail "missing help"
