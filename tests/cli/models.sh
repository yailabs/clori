...
"$YVEX_BIN" evidence models list --registry "$REG" --output table > "$ROOT/list-table.out"
...
"$YVEX_BIN" evidence models list --registry "$REG" --audit > "$ROOT/list-audit.out"
...
"$YVEX_BIN" evidence models use deepseek4-v4-flash-selected-embed --registry "$REG" > "$ROOT/use.out"
grep 'selected: deepseek4-v4-flash-selected-embed' "$ROOT/use.out"
grep 'status: models-selected' "$ROOT/use.out"

"$YVEX_BIN" evidence models current --registry "$REG" > "$ROOT/current.out"
...
"$YVEX_BIN" evidence models current --registry "$REG" --audit > "$ROOT/current-audit.out"
...
"$YVEX_BIN" evidence models list --registry "$REG" --output nope > "$ROOT/list-bad-output.out" 2> "$ROOT/list-bad-output.err" && exit 1 || true
grep 'unsupported output mode: nope' "$ROOT/list-bad-output.err"

"$YVEX_BIN" evidence models inspect deepseek4-v4-flash-selected-embed --registry "$REG" > "$ROOT/inspect.out"
...
"$YVEX_BIN" evidence models inspect deepseek4-v4-flash-selected-embed --registry "$REG" --audit > "$ROOT/inspect-audit.out"
...
"$YVEX_BIN" evidence models inspect deepseek4-v4-flash-selected-embed --registry "$REG" --output nope > "$ROOT/inspect-bad-output.out" 2> "$ROOT/inspect-bad-output.err" && exit 1 || true
grep 'unsupported output mode: nope' "$ROOT/inspect-bad-output.err"

"$YVEX_BIN" evidence models remove deepseek4-v4-flash-selected-embed --registry "$REG" > "$ROOT/remove.out"
grep 'removed: deepseek4-v4-flash-selected-embed' "$ROOT/remove.out"
grep 'status: models-removed' "$ROOT/remove.out"

"$YVEX_BIN" evidence models current --registry "$REG" > "$ROOT/current-none.out"
grep 'selected: none' "$ROOT/current-none.out"
grep 'status: models-none' "$ROOT/current-none.out"

"$YVEX_BIN" evidence models use missing --registry "$REG" > "$ROOT/use-missing.out" 2> "$ROOT/use-missing.err" && exit 1 || true
grep 'alias not found: missing' "$ROOT/use-missing.err"

"$YVEX_BIN" evidence models --help > "$ROOT/help.out"
grep 'yvex evidence models' "$ROOT/help.out"
grep 'evidence models download TARGET' "$ROOT/help.out"
grep -- '--auth auto|required|never' "$ROOT/help.out"
grep -- '--progress auto|live|plain|log|off' "$ROOT/help.out"
grep 'evidence models download status qwen3-32b' "$ROOT/help.out"
grep 'evidence models prepare TARGET' "$ROOT/help.out"
grep 'evidence models prepare TARGET .*--audit' "$ROOT/help.out"
grep 'evidence models check TARGET' "$ROOT/help.out"
...
YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download gemma-4-12b-it --dry-run --models-root "$DOWNLOAD_ROOT" --auth never --audit > "$ROOT/download-dry-run.out"
...
YVEX_FAKE_HF_AUTH=1 YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download gemma-4-12b-it --models-root "$DOWNLOAD_ROOT" --auth auto --audit > "$ROOT/download-gemma.out"
...
YVEX_FAKE_HF_AUTH=1 YVEX_FAKE_HF_STEP_DELAY=1 YVEX_FAKE_HF_STEPS=3 YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download gemma-4-12b-it --models-root "$LIVE_ROOT" --auth required --progress plain --tick-seconds 1 --audit > "$ROOT/download-live.out" 2>&1 &
LIVE_PID=$!
sleep 1
...
YVEX_FAKE_HF_AUTH=1 YVEX_FAKE_HF_FAIL_AT_STEP=2 YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download gemma-4-12b-it --models-root "$LIVE_FAIL_ROOT" --progress plain --tick-seconds 1 --audit > "$ROOT/download-live-fail.out" 2>&1 && exit 1 || true
...
YVEX_FAKE_HF_AUTH=1 YVEX_FAKE_HF_STEP_DELAY=5 YVEX_FAKE_HF_STEPS=8 YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download gemma-4-12b-it --models-root "$SIGNAL_ROOT" --progress plain --tick-seconds 1 --audit > "$ROOT/download-signal.out" 2>&1 &
SIGNAL_PID=$!
sleep 1
kill -INT "$SIGNAL_PID"
set +e
...
YVEX_FAKE_HF_AUTH=1 YVEX_FAKE_HF_STEP_DELAY=5 YVEX_FAKE_HF_STEPS=8 YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download gemma-4-12b-it --models-root "$CONTROL_ROOT" --auth required --progress log --tick-seconds 1 --audit > "$ROOT/download-control-run.out" 2>&1 &
CONTROL_PID=$!
i=0
...
"$YVEX_BIN" evidence models download status gemma-4-12b-it --models-root "$CONTROL_ROOT" --audit > "$ROOT/download-control-status-running.out"
...
"$YVEX_BIN" evidence models download stop gemma-4-12b-it --models-root "$CONTROL_ROOT" --audit > "$ROOT/download-control-stop.out"
...
"$YVEX_BIN" evidence models download status gemma-4-12b-it --models-root "$CONTROL_ROOT" --audit > "$ROOT/download-control-status-stopped.out"
...
YVEX_FAKE_HF_AUTH=1 YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download resume gemma-4-12b-it --models-root "$CONTROL_ROOT" --auth required --progress log --tick-seconds 1 --audit > "$ROOT/download-control-resume.out" 2>&1
...
"$YVEX_BIN" evidence models download status gemma-4-12b-it --models-root "$CONTROL_ROOT" --audit > "$ROOT/download-control-status-resumed.out"
...
EOF
"$YVEX_BIN" evidence models download status gemma-4-12b-it --models-root "$STALE_ROOT" --audit > "$ROOT/download-control-stale-status.out"
...
YVEX_FAKE_HF_AUTH=1 YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download resume gemma-4-12b-it --models-root "$STALE_ROOT" --auth required --audit > "$ROOT/download-control-lock-blocked.out" 2>&1 && exit 1 || true
...
"$YVEX_BIN" evidence models download cleanup gemma-4-12b-it --models-root "$STALE_ROOT" --stale-locks --dry-run --audit > "$ROOT/download-control-cleanup-dry-run.out"
...
"$YVEX_BIN" evidence models download cleanup gemma-4-12b-it --models-root "$STALE_ROOT" --stale-locks --yes --audit > "$ROOT/download-control-cleanup.out"
grep 'status: model-download-cleanup' "$ROOT/download-control-cleanup.out"
grep 'deleted: 1' "$ROOT/download-control-cleanup.out"
test ! -f "$STALE_ROOT/hf/gemma/gemma-4-12b-it/.cache/huggingface/download/model.safetensors.lock"

printf 'lock\n' > "$STALE_ROOT/hf/gemma/gemma-4-12b-it/.cache/huggingface/download/model.safetensors.lock"
YVEX_FAKE_HF_AUTH=1 YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download resume gemma-4-12b-it --models-root "$STALE_ROOT" --auth required --clear-stale-locks --audit > "$ROOT/download-control-lock-clear-resume.out" 2>&1
...
PY
"$YVEX_BIN" evidence models download status gemma-4-12b-it --models-root "$SAFE_ROOT" --audit > "$ROOT/download-control-safe-truncated.out"
...
"$YVEX_BIN" evidence models download status gemma-4-12b-it --models-root "$SAFE_ROOT" --audit > "$ROOT/download-control-safe-ok.out"
grep 'safetensors_size_status: ok' "$ROOT/download-control-safe-ok.out"

YVEX_FAKE_HF_AUTH=1 YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download gemma-4-e2b --models-root "$ROOT/download-off" --no-progress --audit > "$ROOT/download-off.out" 2>&1
...
YVEX_FAKE_HF_AUTH=1 YVEX_FAKE_HF_STEP_DELAY=1 YVEX_FAKE_HF_STEPS=3 YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download gemma-4-e2b-it --models-root "$LOG_PROGRESS_ROOT" --progress log --tick-seconds 1 --audit > "$ROOT/download-log-progress.out" 2>&1
...
YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download gemma-4-e2b --models-root "$DOWNLOAD_ROOT/noauth" --auth never --audit > "$ROOT/download-auth-never.out"
...
YVEX_FAKE_HF_AUTH=1 YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download qwen3-8b --models-root "$DOWNLOAD_ROOT" --auth auto --audit > "$ROOT/download-qwen.out"
...
YVEX_FAKE_HF_AUTH=1 YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download --repo Qwen/Qwen3.6-35B-A3B --family qwen --name qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --auth auto --progress off --audit > "$ROOT/download-dynamic-qwen.out"
...
"$YVEX_BIN" evidence models download status qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --audit > "$ROOT/download-dynamic-qwen-status.out"
...
"$YVEX_BIN" evidence models download cleanup qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --stale-locks --dry-run --audit > "$ROOT/download-dynamic-qwen-cleanup.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --audit > "$ROOT/tensor-map-dynamic-qwen-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --output table > "$ROOT/tensor-map-dynamic-qwen-table.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --role output-head --audit > "$ROOT/output-head-dynamic-qwen-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --role output-head --output table > "$ROOT/output-head-dynamic-qwen-table.out"
...
"$YVEX_BIN" evidence target tokenizer-map qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" > "$ROOT/tokenizer-map-dynamic-qwen.out"
...
"$YVEX_BIN" evidence target tokenizer-map qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --output table > "$ROOT/tokenizer-map-dynamic-qwen-table.out"
...
"$YVEX_BIN" evidence target tokenizer-map qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --audit > "$ROOT/tokenizer-map-dynamic-qwen-audit.out"
...
"$YVEX_BIN" evidence target tokenizer-map qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --output json > "$ROOT/tokenizer-map-dynamic-qwen-json.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --role tokenizer --audit > "$ROOT/tokenizer-map-dynamic-qwen-compat-audit.out"
...
"$YVEX_BIN" evidence target missing-roles qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" > "$ROOT/missing-roles-dynamic-qwen-coverage.out"
...
"$YVEX_BIN" evidence target missing-roles qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --audit > "$ROOT/missing-roles-dynamic-qwen-coverage-audit.out"
...
"$YVEX_BIN" evidence target missing-roles qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --output json > "$ROOT/missing-roles-dynamic-qwen-coverage-json.out"
...
"$YVEX_BIN" evidence models prepare qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --dry-run --audit > "$ROOT/prepare-dynamic-qwen-coverage.out" 2>&1 && exit 1 || true
...
"$YVEX_BIN" evidence target quant-policy qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --role-support > "$ROOT/qtype-role-support-dynamic-qwen.out"
...
"$YVEX_BIN" evidence target quant-policy qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --role-support --output table > "$ROOT/qtype-role-support-dynamic-qwen-table.out"
...
"$YVEX_BIN" evidence target quant-policy qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --role-support --audit > "$ROOT/qtype-role-support-dynamic-qwen-audit.out"
...
expect_rc 2 "$YVEX_BIN" evidence target quant-policy qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --role-support --output json > "$ROOT/qtype-role-support-json.out" 2> "$ROOT/qtype-role-support-json.err"
...
"$YVEX_BIN" evidence target tensor-map qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --audit > "$ROOT/tensor-map-dynamic-qwen-incomplete-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --role output-head --audit > "$ROOT/output-head-dynamic-qwen-incomplete-audit.out"
...
"$YVEX_BIN" evidence target missing-roles qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" > "$ROOT/missing-roles-dynamic-qwen.out"
...
"$YVEX_BIN" evidence target missing-roles qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --output table > "$ROOT/missing-roles-dynamic-qwen-table.out"
...
"$YVEX_BIN" evidence target missing-roles qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --audit > "$ROOT/missing-roles-dynamic-qwen-audit.out"
...
"$YVEX_BIN" evidence target missing-roles qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --output json > "$ROOT/missing-roles-dynamic-qwen-json.out"
...
"$YVEX_BIN" evidence models prepare qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --dry-run --audit > "$ROOT/prepare-dynamic-qwen.out" 2>&1 && exit 1 || true
...
"$YVEX_BIN" evidence models prepare qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --dry-run > "$ROOT/prepare-dynamic-qwen-normal.out" 2>&1 && exit 1 || true
...
YVEX_FAKE_HF_AUTH=1 YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download --repo google/Gemma-4-31B-it --family gemma --name gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --auth auto --progress off --audit > "$ROOT/download-dynamic-gemma.out"
...
"$YVEX_BIN" evidence models download status gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --audit > "$ROOT/download-dynamic-gemma-status.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --audit > "$ROOT/tensor-map-dynamic-gemma-audit.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --output table > "$ROOT/tensor-map-dynamic-gemma-table.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --role output-head --audit > "$ROOT/output-head-dynamic-gemma-audit.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --role output-head --output table > "$ROOT/output-head-dynamic-gemma-table.out"
...
"$YVEX_BIN" evidence target tokenizer-map gemma-4-31b-it --models-root "$DYNAMIC_ROOT" > "$ROOT/tokenizer-map-dynamic-gemma.out"
...
"$YVEX_BIN" evidence target tokenizer-map gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --output table > "$ROOT/tokenizer-map-dynamic-gemma-table.out"
...
"$YVEX_BIN" evidence target tokenizer-map gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --audit > "$ROOT/tokenizer-map-dynamic-gemma-audit.out"
...
"$YVEX_BIN" evidence target tokenizer-map gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --output json > "$ROOT/tokenizer-map-dynamic-gemma-json.out"
...
"$YVEX_BIN" evidence models prepare gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --dry-run --audit > "$ROOT/prepare-dynamic-gemma.out" 2>&1 && exit 1 || true
...
"$YVEX_BIN" evidence target quant-policy gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --role-support > "$ROOT/qtype-role-support-dynamic-gemma.out"
...
"$YVEX_BIN" evidence target quant-policy gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --role-support --output table > "$ROOT/qtype-role-support-dynamic-gemma-table.out"
...
"$YVEX_BIN" evidence target quant-policy gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --role-support --audit > "$ROOT/qtype-role-support-dynamic-gemma-audit.out"
...
write_fake_transformer_safetensors "$DYNAMIC_ROOT/hf/qwen/qwen3-6-35b-a3b/model.safetensors" qwen-coverage BF16
write_fake_tokenizer_sidecars "$DYNAMIC_ROOT/hf/qwen/qwen3-6-35b-a3b" qwen
"$YVEX_BIN" evidence target tensor-map qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" --audit > "$ROOT/tensor-map-dynamic-qwen-restored-audit.out"
"$YVEX_BIN" evidence target tokenizer-map qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" > "$ROOT/tokenizer-map-dynamic-qwen-restored.out"
"$YVEX_BIN" evidence target quant-policy --gate v0.1.0 --models-root "$DYNAMIC_ROOT" --output table > "$ROOT/qtype-role-support-gate-table.out"
...
"$YVEX_BIN" evidence target quant-policy --gate v0.1.0 --models-root "$DYNAMIC_ROOT" --audit > "$ROOT/qtype-role-support-gate-audit.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --audit > "$ROOT/tensor-map-dynamic-gemma-tied-audit.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --role output-head --audit > "$ROOT/output-head-dynamic-gemma-tied-audit.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --role output-head --output table > "$ROOT/output-head-dynamic-gemma-tied-table.out"
...
"$YVEX_BIN" evidence target missing-roles gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --audit > "$ROOT/missing-roles-dynamic-gemma-tied-audit.out"
...
"$YVEX_BIN" evidence models prepare gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --dry-run --audit > "$ROOT/prepare-dynamic-gemma-tied.out" 2>&1 && exit 1 || true
...
"$YVEX_BIN" evidence target tensor-map gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --audit > "$ROOT/tensor-map-dynamic-gemma-incomplete-audit.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --role output-head --audit > "$ROOT/output-head-dynamic-gemma-missing-audit.out"
...
"$YVEX_BIN" evidence target missing-roles gemma-4-31b-it --models-root "$DYNAMIC_ROOT" > "$ROOT/missing-roles-dynamic-gemma.out"
...
"$YVEX_BIN" evidence target missing-roles gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --output table > "$ROOT/missing-roles-dynamic-gemma-table.out"
...
"$YVEX_BIN" evidence target missing-roles gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --audit > "$ROOT/missing-roles-dynamic-gemma-audit.out"
...
"$YVEX_BIN" evidence models prepare gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --dry-run --audit > "$ROOT/prepare-dynamic-gemma-incomplete-map.out" 2>&1 && exit 1 || true
...
"$YVEX_BIN" evidence models prepare gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --dry-run > "$ROOT/prepare-dynamic-gemma-normal.out" 2>&1 && exit 1 || true
...
"$YVEX_BIN" evidence models artifacts list --models-root "$DYNAMIC_ROOT" > "$ROOT/artifacts-list.out"
...
"$YVEX_BIN" evidence models artifacts list --models-root "$DYNAMIC_ROOT" --family qwen --output table > "$ROOT/artifacts-list-qwen-table.out"
...
"$YVEX_BIN" evidence models artifacts list --models-root "$DYNAMIC_ROOT" --audit > "$ROOT/artifacts-list-audit.out"
...
"$YVEX_BIN" evidence models artifacts list --models-root "$DYNAMIC_ROOT" --output json > "$ROOT/artifacts-list-json.out"
grep '"status":"artifacts-list"' "$ROOT/artifacts-list-json.out"
grep '"target_id":"qwen3-6-35b-a3b"' "$ROOT/artifacts-list-json.out"
"$YVEX_BIN" evidence models artifacts status qwen3-6-35b-a3b --models-root "$DYNAMIC_ROOT" > "$ROOT/artifacts-status-qwen.out"
...
"$YVEX_BIN" evidence models artifacts status gemma-4-31b-it --models-root "$DYNAMIC_ROOT" --audit > "$ROOT/artifacts-status-gemma-audit.out"
...
"$YVEX_BIN" evidence models download status qwen3-32b --models-root "$QWEN32_STATUS_ROOT" --audit > "$ROOT/download-qwen32-status.out"
...
"$YVEX_BIN" evidence models download cleanup qwen3-32b --models-root "$QWEN32_CLEANUP_ROOT" --failed-partials --dry-run --audit > "$ROOT/download-qwen32-cleanup-dry-run.out"
...
"$YVEX_BIN" evidence models download cleanup qwen3-32b --models-root "$QWEN32_CLEANUP_ROOT" --failed-partials --yes --audit > "$ROOT/download-qwen32-cleanup.out"
...
YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download gemma-4-12b-it --models-root "$DOWNLOAD_ROOT/required" --auth required --audit > "$ROOT/download-auth-required.out" 2> "$ROOT/download-auth-required.err" && exit 1 || true
...
YVEX_HF_CLI=/missing/hf "$YVEX_BIN" evidence models download gemma-4-12b-it --models-root "$DOWNLOAD_ROOT/missing" --auth auto --audit > "$ROOT/download-missing-hf.out" 2> "$ROOT/download-missing-hf.err" && exit 1 || true
...
YVEX_FAKE_HF_AUTH=1 YVEX_FAKE_HF_FAIL=1 YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download gemma-4-12b-it --models-root "$DOWNLOAD_ROOT/fail" --auth auto --audit > "$ROOT/download-fail.out" 2> "$ROOT/download-fail.err" && exit 1 || true
grep 'status: model-download-fail' "$ROOT/download-fail.out"
test -f "$DOWNLOAD_ROOT/fail/logs/gemma-4-12b-it.download.stderr.log"

YVEX_FAKE_HF_AUTH=1 YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download --repo test-org/test-model --family gemma --name test-model --models-root "$DOWNLOAD_ROOT/direct" --auth auto --audit > "$ROOT/download-direct.out"
...
YVEX_FAKE_GH_AUTH=1 YVEX_GH_CLI="$FAKE_GH" "$YVEX_BIN" evidence models download --provider github --repo test-org/test-model --release v1 --asset '*.gguf' --models-root "$DOWNLOAD_ROOT/github" --auth auto --audit > "$ROOT/download-github.out"
...
"$YVEX_BIN" evidence models download --models-root "$DOWNLOAD_ROOT/parser" > "$ROOT/download-missing-target.out" 2> "$ROOT/download-missing-target.err" && exit 1 || true
grep 'requires TARGET or --repo' "$ROOT/download-missing-target.err"
"$YVEX_BIN" evidence models download --repo test-org/test-model --models-root "$DOWNLOAD_ROOT/parser" > "$ROOT/download-repo-no-family.out" 2> "$ROOT/download-repo-no-family.err" && exit 1 || true
grep 'requires --family' "$ROOT/download-repo-no-family.err"
"$YVEX_BIN" evidence models download --repo test-org/test-model --family llama --models-root "$DOWNLOAD_ROOT/parser" > "$ROOT/download-bad-family.out" 2> "$ROOT/download-bad-family.err" && exit 1 || true
grep 'requires --family deepseek|glm|qwen|gemma' "$ROOT/download-bad-family.err"
"$YVEX_BIN" evidence models download gemma-4-12b-it --models-root "" > "$ROOT/download-empty-root.out" 2> "$ROOT/download-empty-root.err" && exit 1 || true
grep 'models download --models-root value is empty or invalid' "$ROOT/download-empty-root.err"
"$YVEX_BIN" evidence models download gemma-4-12b-it --max-workers 0 > "$ROOT/download-bad-workers.out" 2> "$ROOT/download-bad-workers.err" && exit 1 || true
grep 'requires a positive integer' "$ROOT/download-bad-workers.err"
"$YVEX_BIN" evidence models download gemma-4-12b-it --auth maybe > "$ROOT/download-bad-auth.out" 2> "$ROOT/download-bad-auth.err" && exit 1 || true
grep 'auth requires auto|required|never' "$ROOT/download-bad-auth.err"
"$YVEX_BIN" evidence models download gemma-4-12b-it --progress nope > "$ROOT/download-bad-progress.out" 2> "$ROOT/download-bad-progress.err" && exit 1 || true
grep 'requires auto|live|plain|log|off' "$ROOT/download-bad-progress.err"
"$YVEX_BIN" evidence models download gemma-4-12b-it --tick-seconds 0 > "$ROOT/download-bad-tick.out" 2> "$ROOT/download-bad-tick.err" && exit 1 || true
grep 'tick-seconds requires a positive integer' "$ROOT/download-bad-tick.err"
"$YVEX_BIN" evidence models download gemma-4-12b-it --source s3 > "$ROOT/download-bad-source.out" 2> "$ROOT/download-bad-source.err" && exit 1 || true
grep 'supports hf only' "$ROOT/download-bad-source.err"
"$YVEX_BIN" evidence models download gemma-4-12b-it --auth nope > "$ROOT/download-bad-auth.out" 2> "$ROOT/download-bad-auth.err" && exit 1 || true
grep 'requires auto|required|never' "$ROOT/download-bad-auth.err"
"$YVEX_BIN" evidence models download --provider github --repo test-org/test-model > "$ROOT/download-github-no-asset.out" 2> "$ROOT/download-github-no-asset.err" && exit 1 || true
grep 'requires --asset GLOB' "$ROOT/download-github-no-asset.err"
"$YVEX_BIN" evidence models download gemma-4-12b-it --surprise > "$ROOT/download-unknown-flag.out" 2> "$ROOT/download-unknown-flag.err" && exit 1 || true
grep 'unknown models download option' "$ROOT/download-unknown-flag.err"
"$YVEX_BIN" evidence models download gemma-4-12b-it extra > "$ROOT/download-extra-positional.out" 2> "$ROOT/download-extra-positional.err" && exit 1 || true
grep 'extra positional argument' "$ROOT/download-extra-positional.err"

HF_TOKEN=secret-value YVEX_HF_CLI="$FAKE_HF" "$YVEX_BIN" evidence models download gemma-4-e2b --models-root "$DOWNLOAD_ROOT/token" --auth auto --audit > "$ROOT/download-token.out"
...
"$YVEX_BIN" evidence models prepare deepseek4-v4-flash-selected-embed --dry-run --models-root "$PREP" --registry "$PREP_REG" > "$ROOT/prepare-dry-run.out"
...
"$YVEX_BIN" evidence models prepare deepseek4-v4-flash-selected-embed --models-root "$ROOT/missing-prepare" --registry "$ROOT/missing-prepare/registry/models.local.json" > "$ROOT/prepare-missing.out" 2> "$ROOT/prepare-missing.err" && exit 1 || true
grep 'stage: source-path fail' "$ROOT/prepare-missing.out"
grep 'status: model-prepare-fail' "$ROOT/prepare-missing.out"

"$YVEX_BIN" evidence models prepare deepseek4-v4-flash-selected-embed-rmsnorm --dry-run > "$ROOT/prepare-segment-unsupported.out" 2> "$ROOT/prepare-segment-unsupported.err" && exit 1 || true
...
"$YVEX_BIN" evidence models prepare glm-5.2-official-safetensors --dry-run > "$ROOT/prepare-glm-unsupported.out" 2> "$ROOT/prepare-glm-unsupported.err" && exit 1 || true
...
"$YVEX_BIN" evidence models prepare deepseek4-v4-flash-selected-embed --models-root "$PREP" --registry "$PREP_REG" --overwrite --no-register > "$ROOT/prepare-no-register.out"
...
"$YVEX_BIN" evidence models prepare deepseek4-v4-flash-selected-embed --models-root "$PREP" --registry "$PREP_REG" --overwrite --no-use > "$ROOT/prepare-no-use.out"
...
"$YVEX_BIN" evidence models current --registry "$PREP_REG" > "$ROOT/prepare-current-none.out"
grep 'selected: none' "$ROOT/prepare-current-none.out"

"$YVEX_BIN" evidence models prepare deepseek4-v4-flash-selected-embed --models-root "$PREP" --registry "$PREP_REG" --overwrite > "$ROOT/prepare-register-use.out"
...
"$YVEX_BIN" evidence models verify deepseek4-v4-flash-selected-embed --registry "$PREP_REG" > "$ROOT/prepare-verify.out"
...
"$YVEX_BIN" evidence models verify deepseek4-v4-flash-selected-embed --registry "$PREP_REG" --audit > "$ROOT/prepare-verify-audit.out"
...
"$YVEX_BIN" evidence models verify deepseek4-v4-flash-selected-embed --registry "$PREP_REG" --output nope > "$ROOT/verify-bad-output.out" 2> "$ROOT/verify-bad-output.err" && exit 1 || true
grep 'unsupported output mode: nope' "$ROOT/verify-bad-output.err"

"$YVEX_BIN" evidence models prepare deepseek4-v4-flash-selected-embed --models-root "$PREP" --registry "$PREP_REG" > "$ROOT/prepare-overwrite-refused.out" 2> "$ROOT/prepare-overwrite-refused.err" && exit 1 || true
...
"$YVEX_BIN" evidence models prepare deepseek4-v4-flash-selected-embed --out "$PREP_GGUF" --out-dir "$PREP/gguf/deepseek" > "$ROOT/prepare-invalid.out" 2> "$ROOT/prepare-invalid.err" && exit 1 || true
...
"$YVEX_BIN" artifact emit controlled --out "$CHECK_GGUF" --model-name model-check-test --arch llama --overwrite >/dev/null
"$YVEX_BIN" artifact emit controlled --out "$CHECK_ROOT_GGUF" --model-name model-check-target-root-test --arch llama --overwrite >/dev/null
"$YVEX_BIN" evidence models add --path "$CHECK_GGUF" --alias deepseek4-v4-flash-selected-embed --support-level selected-tensor-materialized --registry "$CHECK_REG" > "$ROOT/check-add.out"
grep 'status: models-added' "$ROOT/check-add.out"

"$YVEX_BIN" evidence models check deepseek4-v4-flash-selected-embed --level quick --registry "$CHECK_REG" > "$ROOT/check-quick-normal.out"
...
"$YVEX_BIN" evidence models check deepseek4-v4-flash-selected-embed --level quick --registry "$CHECK_REG" --audit > "$ROOT/check-quick.out"
...
"$YVEX_BIN" evidence models check deepseek4-v4-flash-selected-embed --level quick --models-root "$CHECK_ROOT" --audit > "$ROOT/check-models-root.out"
...
"$YVEX_BIN" evidence models check glm-5.2-official-safetensors --dry-run > "$ROOT/check-invalid-dry-run.out" 2> "$ROOT/check-invalid-dry-run.err" && exit 1 || true
grep 'unknown models check option' "$ROOT/check-invalid-dry-run.err"

"$YVEX_BIN" evidence models check glm-5.2-official-safetensors --level quick --audit > "$ROOT/check-glm-unsupported.out" 2> "$ROOT/check-glm-unsupported.err" && exit 1 || true
...
"$YVEX_BIN" evidence models check deepseek4-v4-flash-selected-embed-rmsnorm --level quick --audit > "$ROOT/check-segment-unsupported.out" 2> "$ROOT/check-segment-unsupported.err" && exit 1 || true
...
"$YVEX_BIN" evidence models check > "$ROOT/check-invalid-missing-target.out" 2> "$ROOT/check-invalid-missing-target.err" && exit 1 || true
grep 'requires TARGET' "$ROOT/check-invalid-missing-target.err"

"$YVEX_BIN" evidence models check deepseek4-v4-flash-selected-embed --backend > "$ROOT/check-invalid-backend-missing.out" 2> "$ROOT/check-invalid-backend-missing.err" && exit 1 || true
grep 'requires a value' "$ROOT/check-invalid-backend-missing.err"

"$YVEX_BIN" evidence models check deepseek4-v4-flash-selected-embed --backend missing > "$ROOT/check-invalid-backend.out" 2> "$ROOT/check-invalid-backend.err" && exit 1 || true
grep 'unknown backend kind' "$ROOT/check-invalid-backend.err"

"$YVEX_BIN" evidence models check deepseek4-v4-flash-selected-embed --level > "$ROOT/check-invalid-level-missing.out" 2> "$ROOT/check-invalid-level-missing.err" && exit 1 || true
grep 'requires a value' "$ROOT/check-invalid-level-missing.err"

"$YVEX_BIN" evidence models check deepseek4-v4-flash-selected-embed --level impossible > "$ROOT/check-invalid-level.out" 2> "$ROOT/check-invalid-level.err" && exit 1 || true
grep 'unknown models check level' "$ROOT/check-invalid-level.err"

"$YVEX_BIN" evidence models check deepseek4-v4-flash-selected-embed --registry "" > "$ROOT/check-invalid-registry.out" 2> "$ROOT/check-invalid-registry.err" && exit 1 || true
grep 'empty or invalid' "$ROOT/check-invalid-registry.err"

"$YVEX_BIN" evidence models check deepseek4-v4-flash-selected-embed --report-dir "" > "$ROOT/check-invalid-report-dir.out" 2> "$ROOT/check-invalid-report-dir.err" && exit 1 || true
grep 'empty or invalid' "$ROOT/check-invalid-report-dir.err"

"$YVEX_BIN" evidence models check deepseek4-v4-flash-selected-embed --unknown-flag > "$ROOT/check-invalid-unknown.out" 2> "$ROOT/check-invalid-unknown.err" && exit 1 || true
grep 'unknown models check option' "$ROOT/check-invalid-unknown.err"

"$YVEX_BIN" evidence models check deepseek4-v4-flash-selected-embed --output nope > "$ROOT/check-invalid-output.out" 2> "$ROOT/check-invalid-output.err" && exit 1 || true
grep 'unsupported output mode: nope' "$ROOT/check-invalid-output.err"

"$YVEX_BIN" evidence target --help > "$ROOT/model-target-help.out"
grep 'evidence target decision --release v0.1.0' "$ROOT/model-target-help.out"
grep 'evidence target class-profile TARGET' "$ROOT/model-target-help.out"
grep 'evidence target tensor-collection TARGET' "$ROOT/model-target-help.out"
grep 'evidence target tensor-map TARGET' "$ROOT/model-target-help.out"
grep 'evidence target missing-roles TARGET' "$ROOT/model-target-help.out"
...
expect_rc 5 "$YVEX_BIN" evidence target class-profile deepseek4-v4-flash \
...
"$YVEX_BIN" evidence target class-profile qwen3-8b --models-root "$CLASS_MISSING_ROOT" > "$ROOT/model-class-qwen-missing.out"
...
"$YVEX_BIN" evidence target class-profile qwen3-8b --models-root "$CLASS_MISSING_ROOT" --audit > "$ROOT/model-class-qwen-missing-audit.out"
...
"$YVEX_BIN" evidence target tensor-collection qwen3-8b --models-root "$CLASS_MISSING_ROOT" > "$ROOT/tensor-collection-qwen-missing.out"
...
"$YVEX_BIN" evidence target tensor-collection qwen3-8b --models-root "$CLASS_MISSING_ROOT" --output table > "$ROOT/tensor-collection-qwen-missing-table.out"
...
"$YVEX_BIN" evidence target tensor-collection qwen3-8b --models-root "$CLASS_MISSING_ROOT" --audit > "$ROOT/tensor-collection-qwen-missing-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --models-root "$CLASS_MISSING_ROOT" > "$ROOT/tensor-map-qwen-missing.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --models-root "$CLASS_MISSING_ROOT" --output table > "$ROOT/tensor-map-qwen-missing-table.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --models-root "$CLASS_MISSING_ROOT" --audit > "$ROOT/tensor-map-qwen-missing-audit.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --models-root "$CLASS_MISSING_ROOT" > "$ROOT/tensor-map-gemma-missing.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --models-root "$CLASS_MISSING_ROOT" --output table > "$ROOT/tensor-map-gemma-missing-table.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --models-root "$CLASS_MISSING_ROOT" --audit > "$ROOT/tensor-map-gemma-missing-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role output-head --models-root "$CLASS_MISSING_ROOT" > "$ROOT/output-head-qwen-missing.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --role output-head --models-root "$CLASS_MISSING_ROOT" > "$ROOT/output-head-gemma-missing.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --role output-head --models-root "$CLASS_MISSING_ROOT" --output table > "$ROOT/output-head-gemma-missing-table.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --role output-head --models-root "$CLASS_MISSING_ROOT" --audit > "$ROOT/output-head-gemma-missing-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role tokenizer --models-root "$CLASS_MISSING_ROOT" > "$ROOT/tokenizer-map-qwen-missing.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role tokenizer --models-root "$CLASS_MISSING_ROOT" --output table > "$ROOT/tokenizer-map-qwen-missing-table.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role tokenizer --models-root "$CLASS_MISSING_ROOT" --audit > "$ROOT/tokenizer-map-qwen-missing-audit.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --role tokenizer --models-root "$CLASS_MISSING_ROOT" > "$ROOT/tokenizer-map-gemma-missing.out"
...
"$YVEX_BIN" evidence target class-profile gemma-4-12b-it --models-root "$CLASS_MISSING_ROOT" > "$ROOT/model-class-gemma-missing.out"
...
"$YVEX_BIN" evidence target class-profile gemma-4-12b-it --models-root "$CLASS_MISSING_ROOT" --output table > "$ROOT/model-class-gemma-missing-table.out"
...
"$YVEX_BIN" evidence target class-profile gemma-4-12b-it --models-root "$CLASS_MISSING_ROOT" --audit > "$ROOT/model-class-gemma-missing-audit.out"
...
"$YVEX_BIN" evidence target tensor-collection gemma-4-12b-it --models-root "$CLASS_MISSING_ROOT" > "$ROOT/tensor-collection-gemma-missing.out"
...
"$YVEX_BIN" evidence target tensor-collection gemma-4-12b-it --models-root "$CLASS_MISSING_ROOT" --output table > "$ROOT/tensor-collection-gemma-missing-table.out"
...
"$YVEX_BIN" evidence target tensor-collection gemma-4-12b-it --models-root "$CLASS_MISSING_ROOT" --audit > "$ROOT/tensor-collection-gemma-missing-audit.out"
...
"$YVEX_BIN" evidence target class-profile qwen3-8b --source "$QWEN_CLASS_SOURCE" > "$ROOT/model-class-qwen.out"
...
"$YVEX_BIN" evidence target class-profile qwen3-8b --source "$QWEN_CLASS_SOURCE" --output table > "$ROOT/model-class-qwen-table.out"
...
"$YVEX_BIN" evidence target class-profile qwen3-8b --source "$QWEN_CLASS_SOURCE" --audit > "$ROOT/model-class-qwen-audit.out"
...
"$YVEX_BIN" evidence target class-profile qwen3-8b --models-root "$QWEN_CLASS_MODELS_ROOT" --audit > "$ROOT/model-class-qwen-models-root-audit.out"
...
"$YVEX_BIN" evidence target tensor-collection qwen3-8b --source "$QWEN_COLLECTION_SOURCE" > "$ROOT/tensor-collection-qwen.out"
...
"$YVEX_BIN" evidence target tensor-collection qwen3-8b --source "$QWEN_COLLECTION_SOURCE" --output table > "$ROOT/tensor-collection-qwen-table.out"
...
"$YVEX_BIN" evidence target tensor-collection qwen3-8b --source "$QWEN_COLLECTION_SOURCE" --audit > "$ROOT/tensor-collection-qwen-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --source "$QWEN_COLLECTION_SOURCE" > "$ROOT/tensor-map-qwen.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --source "$QWEN_COLLECTION_SOURCE" --output table > "$ROOT/tensor-map-qwen-table.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --source "$QWEN_COLLECTION_SOURCE" --audit > "$ROOT/tensor-map-qwen-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --source "$QWEN_COLLECTION_SOURCE" --check-output-contract normal > "$ROOT/output-contract-qwen-tensor-map-normal.out"
"$YVEX_BIN" evidence target tensor-map qwen3-8b --source "$QWEN_COLLECTION_SOURCE" --check-output-contract table > "$ROOT/output-contract-qwen-tensor-map-table.out"
"$YVEX_BIN" evidence target tensor-map qwen3-8b --source "$QWEN_COLLECTION_SOURCE" --check-output-contract audit > "$ROOT/output-contract-qwen-tensor-map-audit.out"

"$YVEX_BIN" evidence target tensor-map qwen3-8b --role output-head --source "$QWEN_COLLECTION_SOURCE" > "$ROOT/output-head-qwen.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role output-head --source "$QWEN_COLLECTION_SOURCE" --output table > "$ROOT/output-head-qwen-table.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role output-head --source "$QWEN_COLLECTION_SOURCE" --audit > "$ROOT/output-head-qwen-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role output-head --source "$QWEN_COLLECTION_SOURCE" --check-output-contract normal > "$ROOT/output-contract-qwen-output-head-normal.out"
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role output-head --source "$QWEN_COLLECTION_SOURCE" --check-output-contract table > "$ROOT/output-contract-qwen-output-head-table.out"
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role output-head --source "$QWEN_COLLECTION_SOURCE" --check-output-contract audit > "$ROOT/output-contract-qwen-output-head-audit.out"
...
"$YVEX_BIN" evidence target tokenizer-map qwen3-8b --source "$TOKENIZER_COMPLETE_SOURCE" > "$ROOT/tokenizer-map-qwen.out"
...
"$YVEX_BIN" evidence target tokenizer-map qwen3-8b --source "$TOKENIZER_COMPLETE_SOURCE" --output table > "$ROOT/tokenizer-map-qwen-table.out"
...
"$YVEX_BIN" evidence target tokenizer-map qwen3-8b --source "$TOKENIZER_COMPLETE_SOURCE" --audit > "$ROOT/tokenizer-map-qwen-audit.out"
...
"$YVEX_BIN" evidence target tokenizer-map gemma-4-12b-it --source "$TOKENIZER_COMPLETE_SOURCE" > "$ROOT/tokenizer-map-gemma.out"
...
"$YVEX_BIN" evidence target tokenizer-map gemma-4-12b-it --source "$TOKENIZER_COMPLETE_SOURCE" --output table > "$ROOT/tokenizer-map-gemma-table.out"
...
"$YVEX_BIN" evidence target tokenizer-map gemma-4-12b-it --source "$TOKENIZER_COMPLETE_SOURCE" --audit > "$ROOT/tokenizer-map-gemma-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role tokenizer --source "$TOKENIZER_MISSING_SOURCE" --audit > "$ROOT/tokenizer-map-metadata-missing-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role tokenizer --source "$TOKENIZER_INCOMPLETE_SOURCE" --audit > "$ROOT/tokenizer-map-incomplete-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role tokenizer --source "$TOKENIZER_MISMATCH_SOURCE" --audit > "$ROOT/tokenizer-map-mismatch-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role tokenizer --source "$TOKENIZER_MALFORMED_SOURCE" --audit > "$ROOT/tokenizer-map-malformed-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role missing-roles --source "$MISSING_ROLE_COMPLETE_SOURCE" > "$ROOT/missing-role-qwen.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role missing-roles --source "$MISSING_ROLE_COMPLETE_SOURCE" --output table > "$ROOT/missing-role-qwen-table.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role missing-roles --source "$MISSING_ROLE_COMPLETE_SOURCE" --audit > "$ROOT/missing-role-qwen-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role missing-roles --source "$MISSING_ROLE_COMPLETE_SOURCE" --check-output-contract normal > "$ROOT/output-contract-qwen-missing-roles-normal.out"
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role missing-roles --source "$MISSING_ROLE_COMPLETE_SOURCE" --check-output-contract table > "$ROOT/output-contract-qwen-missing-roles-table.out"
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role missing-roles --source "$MISSING_ROLE_COMPLETE_SOURCE" --check-output-contract audit > "$ROOT/output-contract-qwen-missing-roles-audit.out"

"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --role missing-roles --source "$MISSING_ROLE_COMPLETE_SOURCE" > "$ROOT/missing-role-gemma.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --role missing-roles --source "$MISSING_ROLE_COMPLETE_SOURCE" --output table > "$ROOT/missing-role-gemma-table.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --role missing-roles --source "$MISSING_ROLE_COMPLETE_SOURCE" --audit > "$ROOT/missing-role-gemma-audit.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --role missing-roles --source "$MISSING_ROLE_COMPLETE_SOURCE" --check-output-contract normal > "$ROOT/output-contract-gemma-missing-roles-normal.out"
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --role missing-roles --source "$MISSING_ROLE_COMPLETE_SOURCE" --check-output-contract table > "$ROOT/output-contract-gemma-missing-roles-table.out"
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --role missing-roles --source "$MISSING_ROLE_COMPLETE_SOURCE" --check-output-contract audit > "$ROOT/output-contract-gemma-missing-roles-audit.out"

"$YVEX_BIN" evidence target tensor-map qwen3-8b --gate v0.1.0 --source "$MISSING_ROLE_COMPLETE_SOURCE" > "$ROOT/tensor-mapping-gate-qwen.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --gate v0.1.0 --source "$MISSING_ROLE_COMPLETE_SOURCE" --output table > "$ROOT/tensor-mapping-gate-qwen-table.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --gate v0.1.0 --source "$MISSING_ROLE_COMPLETE_SOURCE" --audit > "$ROOT/tensor-mapping-gate-qwen-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --gate v0.1.0 --source "$MISSING_ROLE_COMPLETE_SOURCE" --check-output-contract normal > "$ROOT/output-contract-qwen-gate-normal.out"
"$YVEX_BIN" evidence target tensor-map qwen3-8b --gate v0.1.0 --source "$MISSING_ROLE_COMPLETE_SOURCE" --check-output-contract table > "$ROOT/output-contract-qwen-gate-table.out"
"$YVEX_BIN" evidence target tensor-map qwen3-8b --gate v0.1.0 --source "$MISSING_ROLE_COMPLETE_SOURCE" --check-output-contract audit > "$ROOT/output-contract-qwen-gate-audit.out"

"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --gate v0.1.0 --source "$MISSING_ROLE_COMPLETE_SOURCE" > "$ROOT/tensor-mapping-gate-gemma.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --gate v0.1.0 --source "$MISSING_ROLE_COMPLETE_SOURCE" --output table > "$ROOT/tensor-mapping-gate-gemma-table.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --gate v0.1.0 --source "$MISSING_ROLE_COMPLETE_SOURCE" --audit > "$ROOT/tensor-mapping-gate-gemma-audit.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --gate v0.1.0 --source "$MISSING_ROLE_COMPLETE_SOURCE" --check-output-contract normal > "$ROOT/output-contract-gemma-gate-normal.out"
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --gate v0.1.0 --source "$MISSING_ROLE_COMPLETE_SOURCE" --check-output-contract table > "$ROOT/output-contract-gemma-gate-table.out"
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --gate v0.1.0 --source "$MISSING_ROLE_COMPLETE_SOURCE" --check-output-contract audit > "$ROOT/output-contract-gemma-gate-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role missing-roles --source "$MISSING_ROLE_NO_K_SOURCE" > "$ROOT/missing-role-qwen-no-k.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role missing-roles --source "$MISSING_ROLE_NO_K_SOURCE" --audit > "$ROOT/missing-role-qwen-no-k-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --gate v0.1.0 --source "$MISSING_ROLE_NO_K_SOURCE" > "$ROOT/tensor-mapping-gate-qwen-no-k.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role missing-roles --source "$MISSING_ROLE_NO_HEAD_SOURCE" > "$ROOT/missing-role-qwen-no-head.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role missing-roles --source "$MISSING_ROLE_NO_HEAD_SOURCE" --audit > "$ROOT/missing-role-qwen-no-head-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --gate v0.1.0 --source "$MISSING_ROLE_NO_HEAD_SOURCE" > "$ROOT/tensor-mapping-gate-qwen-no-head.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role missing-roles --source "$MISSING_ROLE_NO_METADATA_SOURCE" > "$ROOT/missing-role-qwen-no-metadata.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role missing-roles --source "$MISSING_ROLE_NO_METADATA_SOURCE" --audit > "$ROOT/missing-role-qwen-no-metadata-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --gate v0.1.0 --source "$MISSING_ROLE_NO_METADATA_SOURCE" > "$ROOT/tensor-mapping-gate-qwen-no-metadata.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role missing-roles --source "$MISSING_ROLE_AMBIG_SOURCE" > "$ROOT/missing-role-qwen-ambiguous.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role missing-roles --source "$MISSING_ROLE_AMBIG_SOURCE" --audit > "$ROOT/missing-role-qwen-ambiguous-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --gate v0.1.0 --source "$MISSING_ROLE_AMBIG_SOURCE" > "$ROOT/tensor-mapping-gate-qwen-ambiguous.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role missing-roles --models-root "$MISSING_ROLE_MISSING_ROOT" > "$ROOT/missing-role-qwen-missing-source.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --gate v0.1.0 --models-root "$MISSING_ROLE_MISSING_ROOT" > "$ROOT/tensor-mapping-gate-qwen-missing-source.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --role missing-roles --models-root "$MISSING_ROLE_MISSING_ROOT" > "$ROOT/missing-role-gemma-missing-source.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --gate v0.1.0 --models-root "$MISSING_ROLE_MISSING_ROOT" > "$ROOT/tensor-mapping-gate-gemma-missing-source.out"
...
"$YVEX_BIN" evidence target quant-policy qwen3-8b --source "$MODEL_TARGET_QTYPE_SOURCE" > "$ROOT/qtype-policy-qwen.out"
...
"$YVEX_BIN" evidence target quant-policy qwen3-8b --source "$MODEL_TARGET_QTYPE_SOURCE" --output table > "$ROOT/qtype-policy-qwen-table.out"
...
"$YVEX_BIN" evidence target quant-policy qwen3-8b --source "$MODEL_TARGET_QTYPE_SOURCE" --audit > "$ROOT/qtype-policy-qwen-audit.out"
...
"$YVEX_BIN" evidence target quant-policy qwen3-8b --source "$MODEL_TARGET_QTYPE_SOURCE" --check-output-contract normal > "$ROOT/output-contract-qwen-qtype-normal.out"
"$YVEX_BIN" evidence target quant-policy qwen3-8b --source "$MODEL_TARGET_QTYPE_SOURCE" --check-output-contract table > "$ROOT/output-contract-qwen-qtype-table.out"
"$YVEX_BIN" evidence target quant-policy qwen3-8b --source "$MODEL_TARGET_QTYPE_SOURCE" --check-output-contract audit > "$ROOT/output-contract-qwen-qtype-audit.out"

"$YVEX_BIN" evidence target quant-policy gemma-4-12b-it --source "$MODEL_TARGET_QTYPE_SOURCE" > "$ROOT/qtype-policy-gemma.out"
...
"$YVEX_BIN" evidence target quant-policy gemma-4-12b-it --source "$MODEL_TARGET_QTYPE_SOURCE" --output table > "$ROOT/qtype-policy-gemma-table.out"
...
"$YVEX_BIN" evidence target quant-policy gemma-4-12b-it --source "$MODEL_TARGET_QTYPE_SOURCE" --audit > "$ROOT/qtype-policy-gemma-audit.out"
...
"$YVEX_BIN" evidence target quant-policy gemma-4-12b-it --source "$MODEL_TARGET_QTYPE_SOURCE" --check-output-contract normal > "$ROOT/output-contract-gemma-qtype-normal.out"
"$YVEX_BIN" evidence target quant-policy gemma-4-12b-it --source "$MODEL_TARGET_QTYPE_SOURCE" --check-output-contract table > "$ROOT/output-contract-gemma-qtype-table.out"
"$YVEX_BIN" evidence target quant-policy gemma-4-12b-it --source "$MODEL_TARGET_QTYPE_SOURCE" --check-output-contract audit > "$ROOT/output-contract-gemma-qtype-audit.out"

"$YVEX_BIN" evidence target quant-policy qwen3-8b --models-root "$MISSING_ROLE_MISSING_ROOT" > "$ROOT/qtype-policy-qwen-missing-source.out"
...
"$YVEX_BIN" evidence target quant-policy qwen3-8b --source "$QTYPE_MISSING_DTYPE_SOURCE" > "$ROOT/qtype-policy-qwen-missing-dtype.out"
...
"$YVEX_BIN" evidence target quant-policy qwen3-8b --source "$MISSING_ROLE_NO_K_SOURCE" > "$ROOT/qtype-policy-qwen-blocked-gate.out"
...
expect_rc 2 "$YVEX_BIN" evidence target quant-policy nope > "$ROOT/qtype-policy-unknown-target.out" 2> "$ROOT/qtype-policy-unknown-target.err"
grep 'qtype-policy: nope \[unsupported\]' "$ROOT/qtype-policy-unknown-target.out"
"$YVEX_BIN" evidence target quant-policy deepseek4-v4-flash-selected-embed > "$ROOT/qtype-policy-unsupported-class.out"
...
"$YVEX_BIN" evidence target quant-policy deepseek4-v4-flash-selected-embed-rmsnorm --role-support > "$ROOT/qtype-role-support-deepseek-selected.out"
...
"$YVEX_BIN" evidence target quant-policy deepseek4-v4-flash-selected-embed-rmsnorm --role-support --audit > "$ROOT/qtype-role-support-deepseek-selected-audit.out"
...
"$YVEX_BIN" evidence target quant-policy glm-5.2-official-safetensors > "$ROOT/qtype-policy-unsupported-family.out"
...
expect_rc 2 "$YVEX_BIN" evidence target quant-policy > "$ROOT/qtype-policy-missing-target.out" 2> "$ROOT/qtype-policy-missing-target.err"
grep 'requires TARGET' "$ROOT/qtype-policy-missing-target.err"
expect_rc 2 "$YVEX_BIN" evidence target quant-policy qwen3-8b --output nope > "$ROOT/qtype-policy-bad-output.out" 2> "$ROOT/qtype-policy-bad-output.err"
grep 'unsupported output mode: nope' "$ROOT/qtype-policy-bad-output.err"
expect_rc 2 "$YVEX_BIN" evidence target quant-policy nope --check-output-contract normal > "$ROOT/qtype-policy-contract-unknown-target.out"
grep 'status: unsupported-target' "$ROOT/qtype-policy-contract-unknown-target.out"
expect_rc 2 "$YVEX_BIN" evidence target quant-policy qwen3-8b --check-output-contract nope > "$ROOT/qtype-policy-contract-bad-mode.out"
grep 'status: unsupported-mode' "$ROOT/qtype-policy-contract-bad-mode.out"
expect_rc 2 "$YVEX_BIN" evidence target quant-policy qwen3-8b --check-output-contract > "$ROOT/qtype-policy-contract-missing-mode.out"
grep 'status: parser-error' "$ROOT/qtype-policy-contract-missing-mode.out"
expect_rc 2 "$YVEX_BIN" evidence target quant-policy qwen3-8b --source > "$ROOT/qtype-policy-missing-source-arg.out" 2> "$ROOT/qtype-policy-missing-source-arg.err"
grep 'source requires DIR' "$ROOT/qtype-policy-missing-source-arg.err"
expect_rc 2 "$YVEX_BIN" evidence target quant-policy qwen3-8b --models-root > "$ROOT/qtype-policy-missing-models-root.out" 2> "$ROOT/qtype-policy-missing-models-root.err"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role output-head --source "$QWEN_OUTPUT_HEAD_MISSING_SOURCE" --audit > "$ROOT/output-head-qwen-missing-head-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --role output-head --source "$QWEN_OUTPUT_HEAD_AMBIGUOUS_SOURCE" --audit > "$ROOT/output-head-qwen-ambiguous-audit.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --source "$QWEN_TENSOR_MAP_UNKNOWN_SOURCE" > "$ROOT/tensor-map-qwen-unknown.out"
...
"$YVEX_BIN" evidence target tensor-map qwen3-8b --source "$QWEN_TENSOR_MAP_UNKNOWN_SOURCE" --audit > "$ROOT/tensor-map-qwen-unknown-audit.out"
...
"$YVEX_BIN" evidence target class-profile gemma-4-12b-it --source "$GEMMA_CLASS_SOURCE" > "$ROOT/model-class-gemma.out"
...
"$YVEX_BIN" evidence target class-profile gemma-4-12b-it --source "$GEMMA_CLASS_SOURCE" --output table > "$ROOT/model-class-gemma-table.out"
...
"$YVEX_BIN" evidence target class-profile gemma-4-12b-it --source "$GEMMA_CLASS_SOURCE" --audit > "$ROOT/model-class-gemma-audit.out"
...
"$YVEX_BIN" evidence target class-profile gemma-4-12b-it --models-root "$GEMMA_CLASS_MODELS_ROOT" --audit > "$ROOT/model-class-gemma-models-root-audit.out"
...
"$YVEX_BIN" evidence target tensor-collection gemma-4-12b-it --source "$GEMMA_CLASS_SOURCE" > "$ROOT/tensor-collection-gemma.out"
...
"$YVEX_BIN" evidence target tensor-collection gemma-4-12b-it --source "$GEMMA_CLASS_SOURCE" --output table > "$ROOT/tensor-collection-gemma-table.out"
...
"$YVEX_BIN" evidence target tensor-collection gemma-4-12b-it --source "$GEMMA_CLASS_SOURCE" --audit > "$ROOT/tensor-collection-gemma-audit.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --source "$GEMMA_CLASS_SOURCE" > "$ROOT/tensor-map-gemma.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --source "$GEMMA_CLASS_SOURCE" --output table > "$ROOT/tensor-map-gemma-table.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --source "$GEMMA_CLASS_SOURCE" --audit > "$ROOT/tensor-map-gemma-audit.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --source "$GEMMA_CLASS_SOURCE" --check-output-contract normal > "$ROOT/output-contract-gemma-tensor-map-normal.out"
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --source "$GEMMA_CLASS_SOURCE" --check-output-contract table > "$ROOT/output-contract-gemma-tensor-map-table.out"
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --source "$GEMMA_CLASS_SOURCE" --check-output-contract audit > "$ROOT/output-contract-gemma-tensor-map-audit.out"

"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --role output-head --source "$GEMMA_CLASS_SOURCE" > "$ROOT/output-head-gemma.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --role output-head --source "$GEMMA_CLASS_SOURCE" --output table > "$ROOT/output-head-gemma-table.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --role output-head --source "$GEMMA_CLASS_SOURCE" --audit > "$ROOT/output-head-gemma-audit.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --role output-head --source "$GEMMA_CLASS_SOURCE" --check-output-contract normal > "$ROOT/output-contract-gemma-output-head-normal.out"
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --role output-head --source "$GEMMA_CLASS_SOURCE" --check-output-contract table > "$ROOT/output-contract-gemma-output-head-table.out"
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --role output-head --source "$GEMMA_CLASS_SOURCE" --check-output-contract audit > "$ROOT/output-contract-gemma-output-head-audit.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --source "$GEMMA_TENSOR_MAP_UNKNOWN_SOURCE" > "$ROOT/tensor-map-gemma-unknown.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --source "$GEMMA_TENSOR_MAP_UNKNOWN_SOURCE" --audit > "$ROOT/tensor-map-gemma-unknown-audit.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --source "$GEMMA_TENSOR_MAP_NORM_SOURCE" > "$ROOT/tensor-map-gemma-norm.out"
...
"$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --source "$GEMMA_TENSOR_MAP_NORM_SOURCE" --audit > "$ROOT/tensor-map-gemma-norm-audit.out"
...
expect_rc 2 "$YVEX_BIN" evidence target class-profile > "$ROOT/model-class-missing-target.out" 2> "$ROOT/model-class-missing-target.err"
grep 'requires TARGET' "$ROOT/model-class-missing-target.err"
expect_rc 2 "$YVEX_BIN" evidence target class-profile nope > "$ROOT/model-class-bad-target.out" 2> "$ROOT/model-class-bad-target.err"
grep 'unsupported target: nope' "$ROOT/model-class-bad-target.err"
expect_rc 2 "$YVEX_BIN" evidence target class-profile qwen-metal-portability > "$ROOT/model-class-old-target.out" 2> "$ROOT/model-class-old-target.err"
grep 'unsupported target: qwen-metal-portability' "$ROOT/model-class-old-target.err"
expect_rc 2 "$YVEX_BIN" evidence target class-profile gemma-dense-portability > "$ROOT/model-class-old-gemma-target.out" 2> "$ROOT/model-class-old-gemma-target.err"
grep 'unsupported target: gemma-dense-portability' "$ROOT/model-class-old-gemma-target.err"
expect_rc 2 "$YVEX_BIN" evidence target class-profile qwen3-8b --output nope > "$ROOT/model-class-bad-output.out" 2> "$ROOT/model-class-bad-output.err"
grep 'unsupported output mode: nope' "$ROOT/model-class-bad-output.err"
expect_rc 2 "$YVEX_BIN" evidence target class-profile qwen3-8b --source > "$ROOT/model-class-missing-source.out" 2> "$ROOT/model-class-missing-source.err"
grep 'source requires DIR' "$ROOT/model-class-missing-source.err"
expect_rc 2 "$YVEX_BIN" evidence target class-profile gemma-4-12b-it --output nope > "$ROOT/model-class-gemma-bad-output.out" 2> "$ROOT/model-class-gemma-bad-output.err"
grep 'unsupported output mode: nope' "$ROOT/model-class-gemma-bad-output.err"
expect_rc 2 "$YVEX_BIN" evidence target class-profile gemma-4-12b-it --source > "$ROOT/model-class-gemma-missing-source.out" 2> "$ROOT/model-class-gemma-missing-source.err"
grep 'source requires DIR' "$ROOT/model-class-gemma-missing-source.err"

expect_rc 2 "$YVEX_BIN" evidence target tensor-collection > "$ROOT/tensor-collection-missing-target.out" 2> "$ROOT/tensor-collection-missing-target.err"
grep 'requires TARGET' "$ROOT/tensor-collection-missing-target.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-collection nope > "$ROOT/tensor-collection-bad-target.out" 2> "$ROOT/tensor-collection-bad-target.err"
grep 'unsupported target: nope' "$ROOT/tensor-collection-bad-target.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-collection qwen-metal-portability > "$ROOT/tensor-collection-old-target.out" 2> "$ROOT/tensor-collection-old-target.err"
grep 'unsupported target: qwen-metal-portability' "$ROOT/tensor-collection-old-target.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-collection qwen3-8b --output nope > "$ROOT/tensor-collection-bad-output.out" 2> "$ROOT/tensor-collection-bad-output.err"
grep 'unsupported output mode: nope' "$ROOT/tensor-collection-bad-output.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-collection qwen3-8b --source > "$ROOT/tensor-collection-missing-source.out" 2> "$ROOT/tensor-collection-missing-source.err"
grep 'source requires DIR' "$ROOT/tensor-collection-missing-source.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-collection qwen3-8b --models-root > "$ROOT/tensor-collection-missing-models-root.out" 2> "$ROOT/tensor-collection-missing-models-root.err"
grep 'models-root requires DIR' "$ROOT/tensor-collection-missing-models-root.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-collection gemma-dense-portability > "$ROOT/tensor-collection-old-gemma-target.out" 2> "$ROOT/tensor-collection-old-gemma-target.err"
grep 'unsupported target: gemma-dense-portability' "$ROOT/tensor-collection-old-gemma-target.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-collection gemma-4-12b-it --output nope > "$ROOT/tensor-collection-gemma-bad-output.out" 2> "$ROOT/tensor-collection-gemma-bad-output.err"
grep 'unsupported output mode: nope' "$ROOT/tensor-collection-gemma-bad-output.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-collection gemma-4-12b-it --source > "$ROOT/tensor-collection-gemma-missing-source.out" 2> "$ROOT/tensor-collection-gemma-missing-source.err"
grep 'source requires DIR' "$ROOT/tensor-collection-gemma-missing-source.err"

expect_rc 2 "$YVEX_BIN" evidence target tensor-map > "$ROOT/tensor-map-missing-target.out" 2> "$ROOT/tensor-map-missing-target.err"
grep 'requires TARGET' "$ROOT/tensor-map-missing-target.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map nope > "$ROOT/tensor-map-bad-target.out" 2> "$ROOT/tensor-map-bad-target.err"
grep 'unsupported target: nope' "$ROOT/tensor-map-bad-target.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen-metal-portability > "$ROOT/tensor-map-old-target.out" 2> "$ROOT/tensor-map-old-target.err"
grep 'unsupported target: qwen-metal-portability' "$ROOT/tensor-map-old-target.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen3-8b --output nope > "$ROOT/tensor-map-bad-output.out" 2> "$ROOT/tensor-map-bad-output.err"
grep 'unsupported output mode: nope' "$ROOT/tensor-map-bad-output.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map nope --check-output-contract normal > "$ROOT/tensor-map-contract-unknown-target.out"
grep 'status: unsupported-target' "$ROOT/tensor-map-contract-unknown-target.out"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen3-8b --check-output-contract nope > "$ROOT/tensor-map-contract-bad-mode.out"
grep 'status: unsupported-mode' "$ROOT/tensor-map-contract-bad-mode.out"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen3-8b --check-output-contract > "$ROOT/tensor-map-contract-missing-mode.out"
...
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen3-8b --source > "$ROOT/tensor-map-missing-source.out" 2> "$ROOT/tensor-map-missing-source.err"
grep 'source requires DIR' "$ROOT/tensor-map-missing-source.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen3-8b --models-root > "$ROOT/tensor-map-missing-models-root.out" 2> "$ROOT/tensor-map-missing-models-root.err"
grep 'models-root requires DIR' "$ROOT/tensor-map-missing-models-root.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --output nope > "$ROOT/tensor-map-gemma-bad-output.out" 2> "$ROOT/tensor-map-gemma-bad-output.err"
grep 'unsupported output mode: nope' "$ROOT/tensor-map-gemma-bad-output.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --source > "$ROOT/tensor-map-gemma-missing-source.out" 2> "$ROOT/tensor-map-gemma-missing-source.err"
grep 'source requires DIR' "$ROOT/tensor-map-gemma-missing-source.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map gemma-dense-portability > "$ROOT/tensor-map-old-gemma-target.out" 2> "$ROOT/tensor-map-old-gemma-target.err"
grep 'unsupported target: gemma-dense-portability' "$ROOT/tensor-map-old-gemma-target.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen3-8b --role > "$ROOT/output-head-missing-role.out" 2> "$ROOT/output-head-missing-role.err"
grep 'role requires output-head|tokenizer' "$ROOT/output-head-missing-role.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen3-8b --role nope > "$ROOT/output-head-qwen-bad-role.out" 2> "$ROOT/output-head-qwen-bad-role.err"
grep 'unsupported role: nope' "$ROOT/output-head-qwen-bad-role.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map gemma-4-12b-it --role nope > "$ROOT/output-head-gemma-bad-role.out" 2> "$ROOT/output-head-gemma-bad-role.err"
grep 'unsupported role: nope' "$ROOT/output-head-gemma-bad-role.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen-metal-portability --role output-head > "$ROOT/output-head-old-qwen-target.out" 2> "$ROOT/output-head-old-qwen-target.err"
grep 'unsupported target: qwen-metal-portability' "$ROOT/output-head-old-qwen-target.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map gemma-dense-portability --role output-head > "$ROOT/output-head-old-gemma-target.out" 2> "$ROOT/output-head-old-gemma-target.err"
grep 'unsupported target: gemma-dense-portability' "$ROOT/output-head-old-gemma-target.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen3-8b --role output-head --output nope > "$ROOT/output-head-bad-output.out" 2> "$ROOT/output-head-bad-output.err"
grep 'unsupported output mode: nope' "$ROOT/output-head-bad-output.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen3-8b --role output-head --source > "$ROOT/output-head-missing-source.out" 2> "$ROOT/output-head-missing-source.err"
grep 'source requires DIR' "$ROOT/output-head-missing-source.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen-metal-portability --role tokenizer > "$ROOT/tokenizer-old-qwen-target.out" 2> "$ROOT/tokenizer-old-qwen-target.err"
grep 'unsupported target: qwen-metal-portability' "$ROOT/tokenizer-old-qwen-target.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map gemma-dense-portability --role tokenizer > "$ROOT/tokenizer-old-gemma-target.out" 2> "$ROOT/tokenizer-old-gemma-target.err"
grep 'unsupported target: gemma-dense-portability' "$ROOT/tokenizer-old-gemma-target.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen3-8b --role tokenizer --output nope > "$ROOT/tokenizer-bad-output.out" 2> "$ROOT/tokenizer-bad-output.err"
grep 'unsupported output mode: nope' "$ROOT/tokenizer-bad-output.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen3-8b --role tokenizer --source > "$ROOT/tokenizer-missing-source.out" 2> "$ROOT/tokenizer-missing-source.err"
grep 'source requires DIR' "$ROOT/tokenizer-missing-source.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen3-8b --role missing-roles --output nope > "$ROOT/missing-role-bad-output.out" 2> "$ROOT/missing-role-bad-output.err"
grep 'unsupported output mode: nope' "$ROOT/missing-role-bad-output.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen3-8b --role missing-roles --source > "$ROOT/missing-role-missing-source.out" 2> "$ROOT/missing-role-missing-source.err"
grep 'source requires DIR' "$ROOT/missing-role-missing-source.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen-metal-portability --role missing-roles > "$ROOT/missing-role-old-qwen-target.out" 2> "$ROOT/missing-role-old-qwen-target.err"
grep 'unsupported target: qwen-metal-portability' "$ROOT/missing-role-old-qwen-target.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map gemma-dense-portability --role missing-roles > "$ROOT/missing-role-old-gemma-target.out" 2> "$ROOT/missing-role-old-gemma-target.err"
grep 'unsupported target: gemma-dense-portability' "$ROOT/missing-role-old-gemma-target.err"
expect_rc 2 "$YVEX_BIN" evidence target missing-roles > "$ROOT/missing-roles-direct-missing-target.out" 2> "$ROOT/missing-roles-direct-missing-target.err"
grep 'requires TARGET' "$ROOT/missing-roles-direct-missing-target.err"
expect_rc 2 "$YVEX_BIN" evidence target missing-roles qwen3-8b --output nope > "$ROOT/missing-roles-direct-bad-output.out" 2> "$ROOT/missing-roles-direct-bad-output.err"
grep 'unsupported output mode: nope' "$ROOT/missing-roles-direct-bad-output.err"
expect_rc 2 "$YVEX_BIN" evidence target missing-roles qwen3-8b --models-root > "$ROOT/missing-roles-direct-missing-root.out" 2> "$ROOT/missing-roles-direct-missing-root.err"
grep 'models-root requires DIR' "$ROOT/missing-roles-direct-missing-root.err"
expect_rc 2 "$YVEX_BIN" evidence target missing-roles qwen3-8b --source > "$ROOT/missing-roles-direct-missing-source.out" 2> "$ROOT/missing-roles-direct-missing-source.err"
grep 'source requires DIR' "$ROOT/missing-roles-direct-missing-source.err"
expect_rc 2 "$YVEX_BIN" evidence target missing-roles qwen-metal-portability > "$ROOT/missing-roles-direct-old-qwen-target.out" 2> "$ROOT/missing-roles-direct-old-qwen-target.err"
grep 'unsupported target: qwen-metal-portability' "$ROOT/missing-roles-direct-old-qwen-target.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen3-8b --gate > "$ROOT/tensor-mapping-gate-missing-value.out" 2> "$ROOT/tensor-mapping-gate-missing-value.err"
grep 'gate requires v0.1.0' "$ROOT/tensor-mapping-gate-missing-value.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen3-8b --gate v9.9.9 > "$ROOT/tensor-mapping-gate-bad-release.out" 2> "$ROOT/tensor-mapping-gate-bad-release.err"
grep 'unsupported release: v9.9.9' "$ROOT/tensor-mapping-gate-bad-release.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen3-8b --gate v0.1.0 --output nope > "$ROOT/tensor-mapping-gate-bad-output.out" 2> "$ROOT/tensor-mapping-gate-bad-output.err"
grep 'unsupported output mode: nope' "$ROOT/tensor-mapping-gate-bad-output.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen-metal-portability --gate v0.1.0 > "$ROOT/tensor-mapping-gate-old-qwen-target.out" 2> "$ROOT/tensor-mapping-gate-old-qwen-target.err"
grep 'unsupported target: qwen-metal-portability' "$ROOT/tensor-mapping-gate-old-qwen-target.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map nope --gate v0.1.0 > "$ROOT/tensor-mapping-gate-unknown-target.out" 2> "$ROOT/tensor-mapping-gate-unknown-target.err"
grep 'unsupported target: nope' "$ROOT/tensor-mapping-gate-unknown-target.err"
expect_rc 2 "$YVEX_BIN" evidence target tensor-map qwen3-8b --role missing-roles --gate v0.1.0 > "$ROOT/tensor-mapping-gate-role-conflict.out" 2> "$ROOT/tensor-mapping-gate-role-conflict.err"
grep 'gate cannot be combined with --role' "$ROOT/tensor-mapping-gate-role-conflict.err"

"$YVEX_BIN" evidence target decision --help > "$ROOT/model-target-decision-help.out"
grep 'usage: yvex evidence target decision --release v0.1.0' "$ROOT/model-target-decision-help.out"
...
"$YVEX_BIN" evidence target decision --release v0.1.0 > "$ROOT/model-target-decision-normal.out"
...
"$YVEX_BIN" evidence target decision --release v0.1.0 --output table > "$ROOT/model-target-decision-table.out"
...
"$YVEX_BIN" evidence target decision --release v0.1.0 --output json > "$ROOT/model-target-decision-json.out"
...
"$YVEX_BIN" evidence target decision --release v0.1.0 --output nope > "$ROOT/model-target-decision-bad-output.out" 2> "$ROOT/model-target-decision-bad-output.err" && exit 1 || true
grep 'model-target decision: unsupported output mode: nope' "$ROOT/model-target-decision-bad-output.err"

"$YVEX_BIN" evidence target decision --release v0.1.0 --audit --include-candidates --include-pressure-targets --include-blockers --include-critical-path --include-next > "$ROOT/model-target-decision.out"
...
"$YVEX_BIN" evidence target decision --release v0.1.0 --audit --candidate deepseek4-v4-flash --include-blockers --include-next > "$ROOT/model-target-decision-deepseek.out"
...
expect_rc 2 "$YVEX_BIN" evidence target decision --release v0.1.0 --audit --candidate deepseek4-v4-flash-selected-embed-rmsnorm --include-blockers --include-next > "$ROOT/model-target-decision-rmsnorm.out" 2> "$ROOT/model-target-decision-rmsnorm.err"
grep 'status: missing-candidate' "$ROOT/model-target-decision-rmsnorm.out"

expect_rc 2 "$YVEX_BIN" evidence target decision --release v0.1.0 --audit --candidate glm-5.2-official-safetensors --include-blockers --include-next > "$ROOT/model-target-decision-glm.out" 2> "$ROOT/model-target-decision-glm.err"
grep 'status: missing-candidate' "$ROOT/model-target-decision-glm.out"

"$YVEX_BIN" evidence target decision --release v0.1.0 --candidate missing-target --include-blockers > "$ROOT/model-target-decision-missing.out" 2> "$ROOT/model-target-decision-missing.err" && exit 1 || true
...
"$YVEX_BIN" evidence target decision --release v9.9.9 > "$ROOT/model-target-decision-unsupported-release.out" 2> "$ROOT/model-target-decision-unsupported-release.err" && exit 1 || true
