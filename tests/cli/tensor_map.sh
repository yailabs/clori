...
"$YVEX_BIN" compile map --arch deepseek4 --native-source "$OUT_DIR/native" --limit 10 > "$OUT_DIR/map.out" 2> "$OUT_DIR/map.err" || fail "tensor-map failed"
grep 'compile map: deepseek4' "$OUT_DIR/map.out" >/dev/null || fail "missing heading"
...
"$YVEX_BIN" compile map --help > "$OUT_DIR/help.out" 2> "$OUT_DIR/help.err" || fail "help tensor-map failed"
grep 'usage: yvex compile map' "$OUT_DIR/help.out" >/dev/null || fail "missing help"
