...
"$YVEX_BIN" compile artifact emit --out "$MODEL" --model-name token-input \
    --arch deepseek --target-qtype F16 --overwrite \
    >"$OUT_DIR/emit.out" 2>"$OUT_DIR/emit.err"

"$YVEX_BIN" inspect input tokens --model "$MODEL" --tokens 0,1 \
...
expect_fail negative "$YVEX_BIN" inspect input tokens --model "$MODEL" --tokens -1
contains "$OUT_DIR/negative.err" "token-parse-invalid"
expect_fail overflow "$YVEX_BIN" inspect input tokens --model "$MODEL" \
